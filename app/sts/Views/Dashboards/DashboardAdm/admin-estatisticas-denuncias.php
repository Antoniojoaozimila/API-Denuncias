    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div id="content">
            ...
            <div class="container-fluid">
                ...
                $denunciasLista = $denuncias->index();
                        ...
                        <table class="table-list" id="denunciasTable">
                            <thead class="list-head">
                                <tr>
                                    ...
                                    <th class="list-head-content">Nome do Técnico</th>
                                    <th class="list-head-content">Telefone</th>
                                    <th class="list-head-content table-sm-none">Denúncias Recebidas</th>
                                    <th class="list-head-content table-sm-none">Denúncias Em Andamento</th>
                                    <th class="list-head-content table-sm-none">Denúncias Resolvidas</th>
                                    <th class="list-head-content">Ações</th>
                                </tr>
                            </thead>
                            <tbody class="list-body">
                                <?php foreach ($this->data as $denuncias) { ?>
                                    <tr>
                                        <td class="list-body-content"><?php echo $denuncias['id'] ?></td>
                                        <td class="list-body-content"><?php echo $denuncias['nome'] ?></td>
                                        <td class="list-body-content"><?php echo $denuncias['telefone'] ?></td>
                                        <td class="list-body-content"><?php echo $denuncias['DRecebidas'] ?></td>
                                        <td class="list-body-content"><?php echo $denuncias['DAndamento'] ?></td>
                                        <td class="list-body-content"><?php echo $denuncias['DResolvidas'] ?></td>
                                        <td class="list-body-content">
                                            <div class="dropdown">
                                                <button class="btn btn-primary dropdown-toggle" type="button" id="dropdownMenuButton<?php echo $denuncias['id'] ?>" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                    Selecione
                                                </button>
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton<?php echo $denuncias['id'] ?>" style="background-color: #ffffff;">
                                                    <?php if (!empty($denunciasLista['denunciasDetails'])) { ?>
                                                        <?php
                                                        $hasOpenDenuncias = false;
                                                        foreach ($denunciasLista['denunciasDetails'] as $denuncia) {
                                                            if ($denuncia['DSTATUS'] === 'Aberta') {
                                                                $hasOpenDenuncias = true;
                                                        ?>
                                                                <a class="dropdown-item" href="javascript:void(0);" onclick="redirecionarDenuncia('<?php echo $denuncias['id']; ?>', '<?php echo $denuncia['id']; ?>')"><?php echo $denuncia['DTITULO']; ?></a>
                                                            <?php
                                                            }
                                                        }
                                                        if (!$hasOpenDenuncias) {
                                                            ?>
                                                            <a class="dropdown-item" href="#">Nenhuma denúncia aberta encontrada</a>
                                                        <?php } ?>
                                                    <?php } else { ?>
                                                        <a class="dropdown-item" href="#">Nenhuma denúncia encontrada</a>
                                                    <?php } ?>
                                                </div>
...
<script>
    function redirecionarDenuncia(tecnicoId, denunciaId) {
        var url = "<?php echo URL; ?>admin-detalhes2-denuncia/" + denunciaId + "?tecnico=" + tecnicoId;
